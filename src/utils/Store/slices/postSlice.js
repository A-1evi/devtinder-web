...
const postSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase("post/fetchPosts/pending", (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase("post/fetchPosts/fulfilled", (state, action) => {
        ...
        if (Array.isArray(action.payload.posts)) {
          ...
          console.error(
            "fetchPosts.fulfilled - Received non-array payload:",
            action.payload
          );
          state.error = {
      ...
      .addCase("post/likePost/fulfilled", (state, action) => {
        ...
        if (postIndex !== -1) {
          // Update likes array and total count
          state.posts[postIndex].likes = action.payload.likes;
          state.posts[postIndex].totalLikes = action.payload.likes.length;
          // Check if current user has liked the post
          state.posts[postIndex].isLiked = action.payload.likes.some(
            (like) => like.user === action.payload.userId
          );
        }
      })
